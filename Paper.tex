...
The variables are represented by lowercase letters ($x,y,r$), parameters by uppercase letters with a different font (\cmss{C}, \cmss{Q}, \cmss{M}, etc.), and sets by uppercase letters with a third font ($\mathcal{N}, \mathcal{K}, \mathcal{Z}$). 
...
$$z^\ast_{\text{RMP}} + \sum_{n \in \mathcal{N}}|\mathcal{N}_n\cdot min(0,z^\ast_{\text{PP}_n})| \leq z^\ast_{\text{MP}}$$

As one of the approaches will not always solve the pricing problems to optimality, the optimal objective value of the pricing problem can
be replaced by its dual bound, whenever available.
...
If the resulting condition is incompatible with the production (see Constraint~\ref{con:production_limit}), then maintenance is scheduled. 
...
A lot more work can go into this heuristic. 
The choice of the production vector in Algorithm~\ref{algo:getProduction} should be more substantiated, and the results mentioned in the beginning of the section regarding maintenance anticipation should also provide better results. 

\todo[inline]{KB: this assumption is important, so I would move it closer to the beginning of the section. 
And may be mention whether the heuristic would still produce any meaningful results for the non-concave case.}
This heuristic also assumes the concavity of function $f$, as otherwise not setting $r^k_t$ to its upper bound (which we do in line $9$ of Algorithm~\ref{algo:PF}) might yield a better solution. 
