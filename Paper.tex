...
\keywords{Production-maintenance scheduling \and Dantzig-Wolfe decomposition \and MINLP \and Resource allocation \and Branch-and-Price}
\end{abstract}
%
%
%
 ...
 \subsubsection{Maintenance in practice}
  ...
  This article is organized as follows: Section~\ref{sec:intro} frames the problem and provides a light background on production-maintenance problems and on maintenance practices, besides presenting the related work.
   In Section~\ref{sec:model}, the compact formulation is presented.
    ...
    This work was funded by the Portuguese Foundation for Science and Technology - FCT with the scholarship 2023.00422.BD, and by Zuse Institute Berlin - ZIB.
...
\label{sec:DW}
    ...
    Dantzig-Wolfe decomposition allows the reformulation of problems with these attributes~\cite{Decomposition_Principle_for_Linear_Programs}, allowing them to be solved with Branch-and-Price. 
    Recently, Desrosiers et al. published a book on Branch-and-Price~\cite{Branch-and-Price}, where one might get a better understanding of the methods mentioned in this paper.
...
This reformulation is solved with column generation, which uses pricing problems to provide new production-maintenance patterns for each of the different sets of machines, with the objective of finding the ones with the minimum reduced cost.
...
If the negative reduced cost columns obtained in this manner are added to the RMP, then we call the model ``DW+PF'' in Section~\ref{sec:results}, otherwise ``DW''.
...
\subsubsection{Branching}
  ...
  As the branching decisions are added to the local restricted master problems, the corresponding duals will also influence the reduced cost of the remaining columns.
  It is then necessary to incorporate them into the objective function of the pricing problem, which becomes

  \begin{mini!}|l|[3]
  {x,y}{C^\intercal x - \pi^\intercal y - \pi_0 - \gamma x}
  {}{}
  \end{mini!}

\subsubsection{Lagrangian bound}
The following is a well known dual bound for the master problem: 

$$c^\ast_{\text{RMP}} + \sum_{z \in \mathcal{Z}}|\mathcal{Z}_z|\cdot \max(0,w_z) \leq c^\ast_{\text{MP}},$$

where $c^\ast_{\text{RMP}}$ is the optimal objective of the restricted master problem, $c^\ast_{\text{MP}}$ is the optimal objective of the master problem, and $w_z$ is a valid dual bound for pricing problem $z$. 

The minimum reduced cost indicates the largest improvement in the objective per unit increase of the corresponding variable (which may not be feasible).
Thus, choosing to add the columns associated to the minimum reduced cost of each subproblem in the RMP yields an upper bound on the improvement of the current iteration. \\
As not all iterations are solved to optimality, it is useful that this remains true for any valid lower bound.
...
\section{Pricing problem heuristics}
...
For that reason, we developed a new heuristic that tries to quickly create columns, which can be used for solving the Farkas' iterations (to initialize the RMP and to fix local infeasibilities), or to add columns with a negative reduced cost.
As referenced in Section~\ref{sec:implementation}, this heuristic may also be helpful in an exact setting, since it can provide an idea of which subproblems are more promissing and should be solved first.
...
\todo[inline]{KB: no need for a separate section to describe the instances. You can make one section for the computational experiments and include this as a subsection. JD@KB:Done.}
 ...
 \todo[inline]{Explain how you generated the instances (what are the nonlinearities?)}
 The instances were randomly generated. 
 They are divided by the number of time periods (either $10$ or $20$), by the number of machines (with some being identical), and they can have ``high'' or ``low'' complexity, meaning that the corresponding machines will have a higher or lower number of components with more or less interaction between them. 
 The complexity of the components also affects the amount of nonlinearity in the model. 
 For each of these combinations, we generate $5$ different instances.

 Functions $f$ and $g$ from the compact formulation and the pricing problem can randomly be linear, polynomial (with a maximum exponent of $4$ for $f$ and $5$ for $g$), or exponential.
 ...
 For this reason, we have opted for creating two sets of instances, with one of them having several identical machines (one group of $20$ machines), and another having two medium-sized groups of identical machines ($2$ sets of $10$ each).
...
The varying combinations of time periods, complexity and machine groups yield $2\cdot 2\cdot 2\cdot 5 = 40$ different instances.
 
For every instance, we ran three formulations: the compact formulation, the extended formulation with exact pricing, and the extended formulation with heuristic pricing (which resorts to exact pricing whenever the heuristic could not find a negative reduced cost column). 
...
The models were solved with the SCIP solver, version 9.1.0~\cite{scip9.0}. 
For the implementation, we used SCIP's Python interface, PySCIPOpt~\cite{PySCIPOPT}, version 5.1.1. Some scripts (most importantly visualization with Matplotlib~\cite{matplotlib}) were implemented in Python version 3.11.4.
...
\begin{tabular}{|c|c|c|c|c|c|}
  \hline
  DW  & Trivial & Easy & Medium & Hard & Impossible\\
  \hline
  Optimal    & 6     & 0     & 0     & -    & -\\
  Timeout    & -     & -     & -     & 12   & 7\\
  Infeasible & 12    & 2     & 1     & -    & -\\
  Gap        & 0     & 0     & 0     & 1275 & -\\
  N\_cols    & 408   & 258   & 342   & 7212 & 2240\\
  Total time & 19.46 & 54.86 & 81.31 & -    & -\\
  \hline
\end{tabular}

\begin{tabular}{|c|c|c|c|c|c|}
  \hline
  DW+PF & Trivial & Easy & Medium & Hard & Impossible\\
  \hline
  Optimal    & 6     & 0     & 0      & -    & -\\
  Timeout    & -     & -     & -      & 13   & 5\\
  Infeasible & 12    & 2     & 2      & -    & -\\
  Gap        & 0     & 0     & 0      & 1576 & -\\
  N\_cols    & 461   & 216   & 963    & 8809 & 1714\\
  Total time & 18.42 & 35.84 & 242.76 & -    & -\\
  \hline
\end{tabular}

\begin{tabular}{|c|c|c|c|c|c|}
  \hline
  Compact & Trivial & Easy & Medium & Hard & Impossible\\
  \hline
  Optimal    & 7    & 1     & 1      & -      & -\\
  Timeout    & -    & -     & -      & 10     & 9\\
  Infeasible & 9    & 1     & 2      & -      & -\\
  Gap        & 0    & 0     & 0      & 2538.7 & -\\
  Total time & 5.36 & 83.46 & 540.65 & -      & -\\
  \hline
\end{tabular}

\vspace{1em}

The compact model is the one that most often could not find a feasible solution in the allocated time, contrasting with heuristic pricing (9 vs 5 instances).\\

The decomposition approaches were better at proving infeasibilities, likely due to Farkas' Pricing.
Both managed to find 12 infeasible instances trivially, while the compact model could only do so for 9 of them.
In total, heuristic pricing proved 16 instances to be infeasible, while Compact could only do it for 12.

The compact formulation found more optimal solutions for the given time limit.
A total of 9 instances (7 trivial, 1 easy, 1 medium) were solved to optimality, against 6 trivial ones by the decomposition approaches.

With an average gap of 106.25\%, the DW algorithm performed the best on the instances that hit the time limit, when comparing with DW+PF (average gap of 121.23\%), and Compact (average gap of 253.87\%).

It is our understanding that the heuristic was overall a good addition to exact pricing.
Heuristic pricing reduced the number of instances for which no solution could be found (from 7 to 5), and could prove infeasibility for an extra instance.
It also does not seem to have added many more columns to the RMP, potentially indicating that more work could increase its effectiveness in finding good columns.\\

From this analysis, we conclude that the compact formulation yielded the worst results, while the decomposition approach with heuristic pricing the best.
...
Besides the compact model, we also developed a Dantzig-Wolfe reformulation, and an additional heuristic to aid with the difficult pricing problem.

    The compact model can quickly solve multiple instances to optimality, but it could not find feasible solutions in nearly $23\%$ of the instances. 
    Both decomposition approaches were generally an improvement, with heuristic pricing yielding the best results out of the three models.
   
    The heuristic should perform even better with additional work.
    Besides the comments at the end of Section~\ref{sec:heuristics}, we are considering taking better advantage of how fast the heuristic is.
  ...
  A different extended formulation seems promising, as it should reduce the number of costly pricing rounds. 
  The idea is to generate maintenance patterns and production patterns (patterns being a concrete assignment to the corresponding variables) in different pricing problems and combine them in the RMP. 
  This formulation needs more theory than the one presented in this paper, as it requires generation of column-and-row generation. 
  See~\cite{Simultaneous_column-and-row_generation} for more details on column-and-row generation.

  On the compact model side, more than one type of maintenance action can make the model more interesting. 
