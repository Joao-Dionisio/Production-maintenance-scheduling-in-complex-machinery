...
Our work differs from the ones we could find, since we are not assuming that our parallel machines are inseparable units, but rather consider them structures composed of interacting components, which can be independently maintained. Furthermore, even working with abstract machines, we admit the possibility of nonlinearities in the degradation of the components, which brings the models closer to reality, and increases their difficulty, as they are mixed-integer nonlinear programs.
 ...
 \subsubsection{Maintenance in practice}
    ...
    To make things worse, these strategies are employed independently of the production decisions, thus further straying away from optimality, which highlights the importance of better maintenance practices. In the literature, some authors focus on optimizing the maintenance decisions, sometimes modeling degradation probabilistically~\cite{markovian_model_liang_2018}.\\


  In terms of applications, this model can be used for minimizing maintenance costs of power transformer fleets, where the ``product'' is electricity, or in water treatment stations, where the ``product'' is clean water. More abstractly, it can also be understood as production-maintenance scheduling in factories working in parallel with different interacting machines. \\

  This article is organized as follows: Section~\ref{sec:intro} frames the problem and provides a light background on production-maintenance problems and on maintenance practices, besides presenting the related work. In Section~\ref{sec:model}, the compact formulation is presented. Section~\ref{sec:DW} shows the Dantzig-Wolfe reformulation of this compact model, alongside implementation details. Due to the difficulty of this model we need some heuristics and consider some simplifications in Section~\ref{sec:heuristics}. Section~\ref{sec:tests} presents the experimental setup and in Section~\ref{sec:results} the results are shown. We conclude the paper in Section~\ref{sec:conclusion}. \\
...
\subsubsection{Branching}
  Branching on fractional master variables is inefficient, and branching on the pricing variables is not possible, due to the aggregation of identical subproblems. 
...
\end{figure}
    ...
    Note, however, that if the bottom solution is feasible, then it is guaranteed to be an optimal solution. By construction, the original solution tells us that component \cmss{B} requires one maintenance action at $y$ at the latest, and component \cmss{A} also requires one at $x$ at the latest.
...
\begin{algorithm}[h]%[H]
  ...
  \Return $r,m,y$\;
  \caption{maintenanceDependencies}
...
After scheduling these maintenance actions, we need to recompute the condition of the components that suffered damage from the production in this time period. Since the production was set to $0$ only when analyzing component $k$, the components that preceded it were incorrectly damaged.
...
This heuristic also assumes the concavity of function $f$, as otherwise ?????. However, it is difficult to describe an example where this would not be the case, in the context of production-maintenance scheduling.
...
\label{sec:tests}
 ...
 The instances were randomly generated. They are divided by the number of time periods (either $10$, $20$, or $50$), by the number of machines, with some being identical, and they can have ``high'' or ``low'' complexity, meaning that the corresponding machines will have a higher or lower number of components, with more or less interaction between them, respectively. The complexity of the components also affects the amount of nonlinearity in the model. For each of these combinations, we generate $10$ different instances.

 It is easy to create instances that favor each of the models, as having more identical machines favors the extended formulation. For this reason, we have opted for creating three sets of instances, with one of them having several identical machines, one other having many dissimilar machines, and an intermediate set.
 
 For every instance, we ran the three formulations with a $2$h time limit each. The tree formulations are the compact formulation, the extended formulation with exact pricing, and the extended formulation with heuristic pricing.

Each of the results represents the average of the $10$ randomly generated instances according to the relevant specification (all with the same complexity, time periods, number of machines). The Time column will show the average time to optimality, for the instances where optimality is achieved, and the Gap (\%) column will show the average Gap for the instances where it was not. The gap is the relative gap: $\frac{|\text{primal bound} - \text{dual bound}|}{ \min(|\text{primal bound}|, |\text{dual bound}|)}$.
...
This article presented a MINLP for deciding how to best allocate production, subjected to a predetermined demand, with the objective of minimizing maintenance cost. Besides the compact model, we also developed a Dantzig-Wolfe reformulation, and additional heuristics due to the model's difficulty. %Even without the heuristic, we can optimally solve more instances (and those we can't, we achieve a much lower gap) than the ones originally achieved by us in~\cite{A_Mixed-Integer_Optimization_Model_for_Efficient_Power_Transformer_Maintenance_and_Operation}, hinting that the reformulations we implemented resulted in a better model. 
